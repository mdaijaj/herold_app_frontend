...
import { SpecificBlogCategoryMain } from "./SpecificCategoryComponents/SpecificBlogCategoryMain";


export const SpecificBlogCategory = () => {
    const { blog } = useParams();
    const { setCounselerIdList, setBlogsList, setGradimoAdviceNow, counselerList, counselerIdList, setIsMobile, setSpecificBlogCategory, setProductsList, baseBackendRoute, setCounselerList } = useContext(AppContext);
    useEffect(() => {
        ...
        const getBlogsCategory = async () => {
            await axios.get(`${baseBackendRoute}/api/v1/blogs/blogs/category/${blog}/`)
                .then((res) => {
                    console.log("king", res);
                    setSpecificBlogCategory(res.data);
                })
                .catch((err) => {
                    console.log(err);
                })
